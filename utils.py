...
class NuscenesUtil():
    ...
    def lane_with_align_coordinate(lane, target_x, target_y, target_yaw):
        return [NuscenesUtil.align_coordinate(target_x, target_y, target_yaw, item[0], item[1], item[2]) for item in
                lane]
    ...
    def get_left_point(target_x, target_y, yaw, offset=3):
        ...
        x, y = NuscenesUtil.rotate(yaw, (x, y), (target_x, target_y))
        return x, y
    ...
    def get_correct_yaw(rotation):
        return NuscenesUtil.quaternion_yaw(Quaternion(rotation))
    ...
    def get_closest_lane(nusc_map, target_x, target_y, target_yaw=None, radius=2):
        ...
        # 获取车辆附近所有的车道
        lanes = NuscenesUtil.get_lanes_in_radius(target_x, target_y, 40, 1, nusc_map)
        for key in lanes:
            lane = lanes[key]
            for index, pos in enumerate(lane):
                distance = NuscenesUtil.point_distance(pos[0], pos[1], target_x, target_y)
                yaw_distance = abs(NuscenesUtil.get_lane_start_yaw(lane[index:]) - target_yaw)
    ...
    # 获取车辆前方的道路，并离散化
    @staticmethod
    def get_front_lane(nusc_map, x, y, yaw=None, _cache=None):
        if _cache is None:
            _cache = NuscenesUtil._cache
        ...
        result = []
        # 获取车辆附近所有的车道
        lanes = NuscenesUtil.get_lanes_in_radius(x, y, 60, 1, nusc_map)
        ...
        while has_next:
            ...
            for lane in lanes.values():
                ...
                # 角度取前3个离散点的平均角度
                yaw_distance = abs(NuscenesUtil.get_lane_start_yaw(lane) - end_point_yaw)
    ...
    def get_front_lane_with_target_coordinate(nusc_map, x, y, yaw=None, target_x=0, target_y=0, target_yaw=0):
        front_lane = NuscenesUtil.get_front_lane(nusc_map, x, y, yaw)
        return NuscenesUtil.lane_with_align_coordinate(front_lane, target_x, target_y, target_yaw)
    ...
    def get_neighbor_lane(nusc_map, x, y, current_lane, offset):
        if len(current_lane) == 0:
            return []
        # 获取当前车道的位置
        pos = NuscenesUtil.get_closest_pos(current_lane, x, y)
        x, y, yaw = current_lane[pos][0], current_lane[pos][1], current_lane[pos][2]
        # 当前车道位置，附近的点
        x, y = offset(x, y, yaw)
        # 根据点获取最近的车道，要求与当前道路不同且方向与当前车道方向一致
        neighbor_lane = NuscenesUtil.get_front_lane(nusc_map, x, y, yaw)
        return neighbor_lane

    # 左侧的车道
    @staticmethod
    def get_left_lane(nusc_map, x, y, current_lane, _cache=None):
        if _cache is None:
            _cache = NuscenesUtil._cache
        ...
        result = NuscenesUtil.get_neighbor_lane(nusc_map, x, y, current_lane, NuscenesUtil.get_left_point)
    ...
    def get_left_lane_with_target_coordinate(nusc_map, x, y, current_lane, target_x, target_y, target_yaw):
        left_lane = NuscenesUtil.get_neighbor_lane(nusc_map, x, y, current_lane, NuscenesUtil.get_left_point)
        return NuscenesUtil.lane_with_align_coordinate(left_lane, target_x, target_y, target_yaw)

    # 右侧的车道
    @staticmethod
    def get_right_lane(nusc_map, x, y, current_lane, _cache=None):
        if _cache is None:
            _cache = NuscenesUtil._cache
        ...
        result = NuscenesUtil.get_neighbor_lane(nusc_map, x, y, current_lane, NuscenesUtil.get_right_point)
    ...
    def get_right_lane_with_target_coordinate(nusc_map, x, y, current_lane, target_x, target_y, target_yaw):
        right_lane = NuscenesUtil.get_neighbor_lane(nusc_map, x, y, current_lane, NuscenesUtil.get_right_point)
        return NuscenesUtil.lane_with_align_coordinate(right_lane, target_x, target_y, target_yaw)

    # 获取附近所有车辆
    @staticmethod
    def get_surrounding_vehicle(helper, nusc_map, instance_token, sample_token, _cache=None):
        if _cache is None:
            _cache = NuscenesUtil._cache
        ...
        # 获取当前车辆所在的道路、左右侧的道路
        lane = NuscenesUtil.get_front_lane(nusc_map, target_x, target_y,
                                           NuscenesUtil.get_correct_yaw(target_ann['rotation']))
        ...
        target_pos = NuscenesUtil.get_closest_pos(lane, target_x, target_y)
        ...
        for ann in annotations:
            ...
            pos = NuscenesUtil.get_closest_pos(lane, x, y)
            relative_pos = pos - target_pos
            pos_on_left = NuscenesUtil.get_closest_pos(left_lane, x, y)
            pos_on_right = NuscenesUtil.get_closest_pos(right_lane, x, y)
            distance_to_lane = NuscenesUtil.point_distance(x, y, lane[pos][0], lane[pos][1])
            ...
            # 左前车、左后车
            if pos_on_left != -1:
                distance_to_left_lane = NuscenesUtil.point_distance(x, y, left_lane[pos_on_left][0],
                                                                    left_lane[pos_on_left][1])
            ...
            # 右前车、右后车
            if pos_on_right != -1:
                distance_to_right_lane = NuscenesUtil.point_distance(x, y, right_lane[pos_on_right][0],
                                                                     right_lane[pos_on_right][1])
    ...
    def get_lane_change(helper, nusc_map, instance_token, sample_token):
        ...
        target_yaw = NuscenesUtil.get_correct_yaw(target_ann['rotation'])
        front_lane = NuscenesUtil.get_front_lane(nusc_map, target_x, target_y, target_yaw)
        left_lane = NuscenesUtil.get_left_lane(nusc_map, target_x, target_y, front_lane)
        right_lane = NuscenesUtil.get_right_lane(nusc_map, target_x, target_y, front_lane)
        # 获取未来六秒的最后一个时间戳
        ...
        for x, y, yaw in front_lane:
            front_distance = min(front_distance, NuscenesUtil.point_distance(x, y, future_x, future_y))
        for x, y, yaw in left_lane:
            left_distance = min(left_distance, NuscenesUtil.point_distance(x, y, future_x, future_y))
        for x, y, yaw in right_lane:
            right_distance = min(right_distance, NuscenesUtil.point_distance(x, y, future_x, future_y))
    ...
    def type_with_one_hot_encoding(type):
        type_list = NuscenesUtil.get_type_list()
